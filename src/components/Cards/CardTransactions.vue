        ...
        <a-list-item>
          ...
          <template v-else>
            <a-list-item-meta :title="item.title" :description="item.datetime">
              <a-avatar size="small" v-if="item.type == 1" style="background-color: #edf9e7">
                <template #avatar>
                  <svg
                    width="10"
                    height="10"
              ...
              <a-avatar size="small" v-if="item.type == 0" style="background-color: #fffce7">
                <template #avatar>
                  <strong class="text-warning"> ! </strong>
                </template>
              </a-avatar>
              <a-avatar size="small" v-if="item.type == -1" style="background-color: #fee9ea">
                <template #avatar>
                  <svg
                    width="10"
                    height="10"
            ...
            <div class="amount">
              <span v-if="item.type == 1" class="text-success"> + ${{ item.amount }} </span>
              <span v-if="item.type == 0" class="text-warning"> Pending </span>
              <span v-if="item.type == -1" class="text-danger"> - ${{ item.amount }} </span>
            </div>
          </template>
        </a-list-item>
      </template>
