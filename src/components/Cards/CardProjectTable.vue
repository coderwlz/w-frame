  ...
  <a-card :bordered="false" class="header-solid h-full" :bodyStyle="{ padding: 0 }">
    <template #title>
      <a-row type="flex" align="middle">
        ...
        >
          <a-radio-group v-model:value="projectHeaderBtns" size="small">
    ...
    <a-table :columns="columns" :data-source="data" :pagination="false">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'company'">
          <h6 class="m-0">
            <img :src="record.company.logo" width="25" class="mr-10" />
            {{ record.company.name }}
          </h6>
        </template>
        <template v-if="column.dataIndex === 'completion'">
          <span class="font-bold text-muted text-sm">{{
            record.completion.label ? record.completion.label : record.completion
          }}</span>
          <a-progress
            :percent="record.completion.value ? record.completion.value : record.completion"
            :show-info="false"
            size="small"
